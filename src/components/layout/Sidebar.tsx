...
const Sidebar = () => {
  const { isOpen, closeMenu, sidebarOpen, setSidebarOpen  } = useMobileMenu();
  ...
  return (
    <>
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-20'} hidden md:flex flex-col duration-300 transition-all bg-gray-900 text-white relative`}>
        ...
        {!sidebarOpen && (
          <div className="mt-auto p-4 flex justify-center">
            <button
              onClick={logout}
              className="p-2 text-gray-400 rounded-lg hover:bg-gray-800 hover:text-white"
            >
              <LogOut size={20} />
            </button>
          </div>
      ...
      {/* Mobile Sidebar */}
      <div 
        className={`fixed inset-0 bg-gray-600 bg-opacity-75 transition-opacity duration-300 ease-in-out z-20 md:hidden ${
      ...
      <aside 
        className={`fixed inset-y-0 left-0 w-64 bg-gray-900 transform transition-transform duration-300 ease-in-out z-30 md:hidden ${
