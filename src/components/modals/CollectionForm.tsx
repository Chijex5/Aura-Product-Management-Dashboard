...
const CollectionForm: React.FC<CollectionFormProps> = ({ 
    ...
    if (isEditingCollection && currentCollection) {
      ...
      setSelectedProductIds(
        currentCollection.products
          .filter(p => p.id !== currentCollection.mainProductId)
          .map(p => p.id)
      );
  ...
  const mainProduct = mainProductId 
    ? products.find(p => p.id === mainProductId) 
    : null;
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {selectedProductIds.length > 0 ? (
              <div className="flex flex-wrap gap-2">
                {selectedProductIds.map(id => {
                  ...
                  return (
                    <div key={id} className="flex items-center bg-gray-100 px-2 py-1 rounded-lg">
              ...
              <p className="text-sm text-gray-500">No additional products selected</p>
            )}
            
            <div className="mt-2 text-sm">
              <span className={`font-medium ${
                mainProductId && selectedProductIds.length >= 1 && selectedProductIds.length <= 3 
                  ? 'text-green-600' 
                  : 'text-gray-500'
              }`}>
              ...
              {mainProductId && selectedProductIds.length >= 1 && selectedProductIds.length <= 3 && (
                <span className="ml-2 text-green-600">✓ Valid selection</span>
              )}
              {(!mainProductId || selectedProductIds.length < 1 || selectedProductIds.length > 3) && (
                <span className="ml-2 text-amber-600">
                  {!mainProductId ? 'Select a main product' : 
                   selectedProductIds.length < 1 ? 'Select at least 1 additional product' :
                   'Maximum 3 additional products allowed'}
                </span>
              )}
            </div>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => handleSetMainProduct(product.id)}
                        className={`p-1.5 rounded-full ${
                          isMainProduct
                            ? 'bg-purple-600 text-white'
                            : 'bg-gray-100 text-gray-500 hover:bg-purple-100'
                        }`}
                        title={isMainProduct ? 'Main product' : 'Set as main product'}
                      >
                        <Check size={16} />
                      </button>
                      <button
                        ...
                        title={
                          isMainProduct ? 'Main product cannot be selected here' :
        ...
        <div className="md:col-span-2 flex justify-end space-x-4 mt-6">
          ...
          <button
            ...
            className="inline-flex justify-center items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:bg-purple-300"
          >
            {isLoading ? (
              <>
